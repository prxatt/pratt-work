...
float pattern(vec2 p, out vec2 q, out vec2 r) {
  ...
  q = vec2(
    fbm(rotSlow * p * 0.8 + flow),
    fbm(rotFlow * p * 0.8 + flow * 0.7 + vec2(3.0))
  );
  
  // Secondary flow layer
  r = vec2(
    fbm(rotSlow * q + flow * 0.5 + vec2(time * 0.015)),
    fbm(q + flow * 0.3)
  );
  
  // Combine with flowing movement
  return fbm(p * 0.9 + r * 0.6 + flow * 0.4);
}

vec2 repulseFromMouse(vec2 p) {
    if(uUseMouse < 0.5) return p;
    
    vec2 mouseWorld = uMouse * uScale;
    vec2 delta = p - mouseWorld;
    float dist = length(delta);
    
    // Subtle meditative repulsion
    float radius = 3.0;
    float strength = uMouseStrength * 1.5;
    
    if(dist < radius && dist > 0.001) {
        float falloff = 1.0 - smoothstep(0.0, radius, dist);
        vec2 repulsionDir = normalize(delta);
        float repulsionAmount = falloff * strength * (radius - dist);
        p += repulsionDir * repulsionAmount;
    }
...
float digit(vec2 p){
    // Apply mouse repulsion to position before grid calculation
    vec2 repulsedP = repulseFromMouse(p);
    
    vec2 grid = uGridMul * 15.0;
    vec2 s = floor(repulsedP * grid) / grid;
    p = repulsedP * grid;
    vec2 q, r;
    ...
    if(uUseMouse > 0.5){
        ...
        float mouseGlow = exp(-distToMouse * 4.0) * uMouseStrength * 2.0;
        intensity += mouseGlow * 0.2;
        
        // Slow meditative ripple
        float ripple = sin(distToMouse * 6.0 - iTime * 1.5) * 0.08 * mouseGlow;
        intensity += ripple;
    }
