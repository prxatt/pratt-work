...
export const Hero = () => {
  ...
  const mobileTerminalScale = isTouch ? (prefersReducedMotion ? 1.18 : 1.28) : 1.62;
  const mobileGrid = isTouch ? [2.42, 2.42] as [number, number] : [2.34, 2.3] as [number, number];
      ...
      <div className="absolute inset-0 z-[1] will-change-transform" style={{ contain: 'strict' }}>
        {showTerminal ? (
          <FaultyTerminal
            ...
            glitchAmount={1}
            flickerAmount={0}
            noiseAmp={isTouch ? 0.36 : lowMotionShader ? 0.28 : 0.34}
            curvature={isTouch ? 0.088 : lowMotionShader ? 0.062 : 0.078}
