...
function smoothDampFloat(
  ...
  change = Math.sign(change) * Math.min(Math.abs(change), maxSpeed * st);
...
export const GridScan = ({
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      isPausedRef.current = document.hidden || !isVisible;
    };
  ...
  }, [uiFaceActive, snapBackDelay, scanOnClick, pushScan]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      frameCount++;
      if (frameCount % 2 === 0 && window.matchMedia('(pointer: coarse)').matches) {
        rafRef.current = requestAnimationFrame(tick);
        return;
      }
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      const detect = async (ts: number) => {
        ...
        if (ts - lastDetect >= 66) { // ~15fps for face detection
          lastDetect = ts;
          try {
            ...
            if (res && res.detection) {
              ...
              const nx = (cx / vw) * 2 - 1;
              const ny = (cy / vh) * 2 - 1;
              medianPush(bufX.current, nx, 3);
              medianPush(bufY.current, ny, 3);
              ...
              medianPush(bufT.current, tilt, 3);
              ...
              medianPush(bufYaw.current, yawSignal, 3);
